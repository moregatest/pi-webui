...
> **生態系角色**：給公司後勤＋客戶的前端接口（沙盒），底層走 Codex + readyAI。
...
## 測試 pi 對 Codex skill 的相容性

升級 pi-coding-agent dist / 換 model / 改 server runtime / 反饋說「pi 跟 Codex 跑同一 skill 不一樣」,**必須**跑這套 regression。

完整流程、prerequisites、對照表格式、已知退化點(Chrome MCP、`pi --print` 互動)都寫在 `.Codex/skills/pi-skill-compat-test/SKILL.md`。讀那份 skill 再開跑,不要憑印象重新發明流程。
