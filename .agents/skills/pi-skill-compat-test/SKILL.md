...
description: Use when verifying pi runs Codex skills equivalently to Codex, regression-testing after pi SDK or pi-coding-agent dist upgrades, evaluating a new LLM (deepseek/qwen/etc.) for skill compliance, or investigating reports that pi behaves differently from Codex on a specific skill
---

# pi 對 Codex skill 相容性測試

## Overview

readyai-webui 的產品定位是「工程師用 Codex 把困難的設計與初版實作做完(skill 內含完整規則 / 決策樹 / 產出規格),後勤人員用 readyai-webui 接手做微調與重跑」。要讓這分工成立,pi 必須把 SKILL.md 當**一級指令文件**:
...
驗證方法:**1:1 重跑比對** — 取一個 Codex 已跑過、產出可信的真實 session,在隔離環境用 pi 跑同一份 skill、同一份輸入,逐 Step / 逐規則比對輸出。
...
- 反饋說「pi 跑 skill 跟 Codex 不一樣」
- 評估某個 skill 是否能放心交給後勤人員用 readyai-webui 重跑
- 新增 skill 後想確認 pi / Codex 兩邊一致
...
3. **`.agents/skills -> .Codex/skills` symlink 存在**

   pi 預設只掃 `~/.agents/skills/`,**不會** auto-discover Codex 的 `.Codex/skills/`。專案層級必須建 symlink:

   ```bash
   # 在 customer 專案根目錄
   ls -la .agents/skills  # 應該 → ../.Codex/skills
   ```
...
從 Codex(Opus 4.7 或當代最強 model)已跑過、產出可信的 session 挑一個。建議優先用 readyaiJobs 流程的真實客戶 case,因為:
...
| 沒設 `.agents/skills -> .Codex/skills` symlink | pi 根本載不到 skill,測出來都是 ❌,但根因是環境 |
...
| 拿不同客戶 case 對 Codex / pi 兩邊 | baseline 不一致,比對毫無意義。一定要同一份輸入 |
